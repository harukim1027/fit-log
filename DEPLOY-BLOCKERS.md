...
항목 1이 해소될 때까지 아래 셋을 지킨다. **운영에 실사용자 데이터가 있고
...
2. **`fix/weekly-goal-default`를 머지하지 않는다.**
   이 브랜치에 `user.entity.ts` 변경이 들어 있다(`weeklyGoal`에 `default: 4`).
   변경 자체는 로컬에서 무해하게 왕복되는 것을 확인했지만, 머지가 곧 배포이고
   배포는 **전체 엔티티를 운영 스키마와 대조**하는 일이라 이 한 줄만의 문제가 아니다.
3. **엔티티 변경이 필요해지면 마이그레이션 인프라를 먼저 세운다.**
   "이번 것만 작으니까"로 예외를 열지 않는다.

> `feat/migrate-iconbutton-callsites`는 한때 `fix/weekly-goal-default` 위에
> rebase되어 있어 같은 엔티티 변경을 업고 있었다. main 위로 되돌려
> **api 변경 0건**임을 확인했으므로 이 브랜치는 규칙 2에 걸리지 않는다.

---

## 1. `synchronize: true`가 운영에서 켜져 있다
...
**"배포 전에 막는 것"이 아니라 지금 열려 있는 위험이다.** TestFlight 배포가
이미 나갔고 운영에 실사용자가 있다.
