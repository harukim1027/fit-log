...
function onWash(bg: string): string {
  return contrastRatio(bg, darkColors.onAccent) >=
    contrastRatio(bg, lightColors.onAccent)
    ? darkColors.onAccent
...
function getHeroCase(
  ...
  if (sessions.some((s) => s.date.slice(0, 10) === todayYMD))
    return "done_today";
...
function HomeScreen() {
  ...
  const slideAnims = useRef(
    [0, 1, 2, 3].map(() => new Animated.Value(24))
  ).current;
  ...
  const { prEntry, prSessionDate, weekMuscles } = useMemo(() => {
    ...
    const prevSessions = sessions.filter(
      (s) => new Date(s.date + "T00:00:00") < start
    );
    ...
    for (const sess of [...weekSessions].sort((a, b) =>
      ...
      for (const ex of sess.exercises) {
        ...
        for (const st of ex.sets) {
          if (st.weight <= 0 || st.reps <= 0) continue;
          const wKg = toKg(st.weight, st.unit);
          if (!best || wKg > best.weightKg)
            best = { weightKg: wKg, reps: st.reps };
        }
    ...
    for (const sess of weekSessions) {
      for (const ex of sess.exercises) {
        const slugs =
          MUSCLE_MAP[ex.name] ?? CATEGORY_TO_SLUGS[ex.category ?? ""] ?? [];
    ...
    return {
      prEntry,
      prSessionDate: prEntry?.date ?? null,
      weekMuscles: Array.from(weekMuscleSet),
    };
  }, [sessions, selectedDate]);
  ...
  const recentSessions = useMemo(() => {
    ...
    const filtered =
      filter === "전체"
        ? completed
        : completed.filter((s) =>
            s.exercises.some((e) => e.category === filter)
          );
    return filtered.slice(0, 6);
  }, [sessions, activeSession, filter]);
  const recentTotal = useMemo(
    () =>
      sessions.filter((s) => !activeSession || s.id !== activeSession.id)
        .length,
  ...
  const muscleHint =
    weekMuscles.length === 0
      ? "이번 주 첫 운동을 기록해보세요"
      : missingMajor
      ? `${MUSCLE_LABELS[missingMajor as Slug] ?? missingMajor}${eunNeun(
          MUSCLE_LABELS[missingMajor as Slug] ?? missingMajor
        )} 이번 주 아직이에요!`
      : "전신 골고루 자극했어요!";
  ...
  const todayData = useMemo(() => {
    ...
    const totalVolume = todaySessions.reduce(
      (sum, s) => sum + calcSessionVolume(s),
      0
    );
    return {
      totalVolume: Math.round(totalVolume).toLocaleString(),
      totalSets,
      exerciseCount,
    };
  }, [sessions, todayYMD]);
    ...
    switch (heroCase) {
      case "active":
        return {
          label: "진행 중",
          headline:
            activeData && activeData.mainCategory
        ...
        {weekDays.map((d) => {
          ...
          return (
            <TouchableOpacity
              ...
              accessibilityLabel={`${d.dow}요일 ${d.num}일${
              ...
              <Text
                style={{
                  fontSize: 12,
                  fontWeight: "600",
                  color: isSun ? c.tagCoral : c.textSecondary,
                }}>
                {d.dow}
              </Text>
              <View
