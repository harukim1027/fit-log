    ...
    <View style={s.container}>
      ...
      <View style={s.presets}>
        {[1500, 1800, 2000, 2500].map(cal => (
          ...
          >
            <Text style={[s.presetText, value === String(cal) && s.presetTextActive]}>{cal}</Text>
          </TouchableOpacity>
        ))}
      </View>
...
const s = StyleSheet.create({
  ...
  label: { fontSize: 16, fontWeight: '600', color: Colors.textSecondary, marginBottom: 12 },
  input: { backgroundColor: Colors.surface, borderRadius: 14, padding: 18, color: Colors.textPrimary, fontSize: 32, fontWeight: '700', textAlign: 'center', borderWidth: 1, borderColor: Colors.border, marginBottom: 8 },
  hint: { fontSize: 13, color: Colors.textMuted, textAlign: 'center', marginBottom: 24 },
  presets: { flexDirection: 'row', gap: 10, marginBottom: 32 },
  preset: { flex: 1, backgroundColor: Colors.surface, borderRadius: 12, paddingVertical: 12, alignItems: 'center', borderWidth: 1, borderColor: Colors.border },
