...
| 홈 "최근 기록" 전역 표시 불일치 | mobile | **정체 규명됨** — 두 갈래다(아래) |
| 계정 삭제(탈퇴) 없음 | api + mobile | **출시 전 필요** — 서버 엔드포인트부터(아래) |
| "최근 기록" 같은 날 순서 미정 | api + mobile | 미해결 — 서버 DTO에 `createdAt` 노출 필요(아래) |
| 날짜 집계 기준 불일치 | mobile | ✅ **해소** — 주 시작 일요일 통일, 유틸 단일화(아래) |

### 재조사가 필요한 항목

**이 문서의 모든 수치에는 잰 시점이 붙어 있어야 한다.** 아래 셋은 그것이
...
- **홈 "최근 기록" 전역 표시 불일치** — 날짜 집계 조사에서 **정체가 드러났다.**
  두 갈래이고 성격이 다르다. 아래 "최근 기록" 절 참조.
...
같은 "이번 주"인데 화면마다 다른 7일을 봤다. 조사에서 **주 시작 요일이
둘**(홈 일요일 / 통계·달력 월요일)이고 **같은 이름의 함수가 두 개**라는 것이
드러났다(2026-09-07, `195edcd` 기준).

| 날짜 | 홈이 보던 주 | 통계가 보던 주 |
|---|---|---|
| 2026-09-05 (토) | 08-30 ~ 09-05 | 08-31 ~ 09-06 |
| **2026-09-06 (일)** | **09-06 ~ 09-12** | **08-31 ~ 09-06** — 겹치는 날 하루 |
| 2026-09-07 (월) | 09-06 ~ 09-12 | 09-07 ~ 09-13 |

**일요일로 통일했다.** ISO 8601은 월요일 시작이고 바꿀 코드도 그쪽이 적었지만
(통계·달력 2곳 대 홈 1곳), 한국 달력이 일요일 시작이고 사용자가 매일 보는 홈
주간 스트립이 이미 일~토였다. **코드량 차이는 한 번이고 어긋남은 매일 겪는다.**

주 범위 계산은 `utils/date.ts` 한 곳에만 둔다.

```
getWeekRange(anchor?)          기준 날짜가 속한 주(일~토). 문자열·Date 둘 다
getWeekRangeByOffset(offset)   오늘로부터 offset주. 위 함수를 호출한다
weekDates(start)               주의 7일을 일~토 순서로
```

시그니처를 둘로 나눈 이유는 홈이 `selectedDate`라는 **날짜**를, 통계가 오늘
로부터의 **offset**을 기준으로 삼기 때문이다. 한 함수에 두 의미를 섞으면
호출부에서 무엇을 넘기는지 읽히지 않는다. offset 쪽이 날짜 쪽을 호출하므로
**주 시작 규칙은 여전히 한 곳뿐이다.**

`toYMD`(홈)·`ymd`(통계)는 `localDateStr`로 흡수했다. 셋이 같은 함수였다.

#### 기준일은 일부러 통일하지 않았다

통계는 오늘 기준, 홈은 `selectedDate` 기준이다. **탭 간 상태를 공유하지
않는다** — 탭을 옮겼는데 이전 탭의 상태가 따라오면 예측하기 어렵다.
홈에서 지난주로 옮겨도 통계는 이번 주를 보여주는 것이 의도다.

대신 홈 라벨을 표시 중인 주에 맞췄다. 지난주를 보면서 "이번 주 운동"을 읽는
상태였다. 이번 주일 때만 접두어를 붙이고 다른 주에서는 뗀다 — 헤더가 이미
"8월 5째주"를 크게 띄우므로 "8월 5째주 운동"은 같은 말을 두 번 하는 것이다.

#### 타임존은 원래 안전했다

`utils/date.ts`와 서버 `common/date.util.ts` 둘 다 주석에
`toISOString()` 함정을 적어 두고 로컬/KST 기준으로 계산한다. 앱 코드에
`toISOString().split()` 사용은 **0건**이고, `new Date(...)`를 `T00:00:00`
없이 파싱하는 곳도 **0건**이다.

> 다만 **서버의 `kstDateStr`은 KST 고정 오프셋**이다. 기기가 다른 타임존이면
> 서버가 계산한 "오늘"과 클라이언트의 "오늘"이 어긋난다. 지금 영향받는 곳은
> 종목 비교 컷오프(`workout.service.ts`의 7·30일)와 물·식단의 날짜 기본값이다.
> 국내 사용자만 있으면 드러나지 않지만 **해외 사용자가 생기면 봐야 한다.**

### 홈 "최근 기록" — 두 갈래의 문제

항목 이름만 있고 내용이 없던 것이 날짜 집계 조사에서 드러났다
(2026-09-07, `195edcd` 기준). **성격이 다른 둘이 한 이름에 묶여 있었다.**

#### (1) 주 단위 화면에 전역 목록이 섞여 있다 — 의도된 것으로 보인다

홈의 다른 지표는 전부 **표시 중인 주**에 종속된다(`운동 N/M`, `자극 부위 N/6`,
PR 배지). 그런데 "최근 기록"만 `sessions` 전체에서 최신 6개를 뽑는다
(`index.tsx`의 `recentSessions`). 지난주로 스와이프해도 목록은 그대로다.

이건 버그라기보다 **다른 의도**로 보인다 — "최근"은 주와 무관한 개념이다.
다만 주 단위 화면 안에 있어 사용자가 "이 주의 기록"으로 오해할 여지가 있다.
주간 지표 라벨을 표시 주에 맞추면서(`0385d37`) 대비가 더 뚜렷해졌다:
같은 화면에서 위쪽은 "운동 0/4"(그 주), 아래쪽은 "최근 기록"(전역)이다.
**Phase 2 홈 재설계에서 함께 볼 것.** 지금 바꾸면 재설계 때 또 바꾸게 된다.

#### (2) 같은 날 기록의 순서가 정해지지 않는다 — 실제 결함

`workout_sessions.date`는 PG `date` 타입이라 **시각이 없다**
(`workout-session.entity.ts:10`). 클라이언트는 `b.date.localeCompare(a.date)`
로만 정렬하고 서버도 `order: { date: 'DESC' }`뿐이다.
**하루에 두 번 운동하면 둘 중 무엇이 위에 올지 정해지지 않는다.**

고치는 데 마이그레이션은 필요 없다. `createdAt` 컬럼이 이미 있다
(베이스라인에 `"createdAt" TIMESTAMP NOT NULL DEFAULT now()`).
남은 일은 셋이다.

- 서버: 세션 응답 DTO에 `createdAt` 노출, `order`를 `date DESC, createdAt DESC` 로
- 클라이언트: `types/workout.ts`의 `WorkoutSession`에 `createdAt` 추가
- 클라이언트: `recentSessions` 정렬을 `date` → `date, createdAt` 순으로

**서버 DTO 변경이라 이번 날짜 집계 작업(`fix/date-aggregation-consistency`)에는
넣지 않았다.** 클라이언트만 고칠 수 있는 문제가 아니다.

### 계정 삭제(탈퇴) — 해소

**App Store Review Guidelines 5.1.1(v) 요구사항을 충족한다.** 계정을 만들 수
있는 앱은 앱 안에서 계정 삭제도 할 수 있어야 하는데, 설정 탭 → 계정 구획의
"회원 탈퇴"가 그 경로다 (2026-09-07, `e147a8b` + 이번 커밋 기준).

전에는 **서버에 엔드포인트 자체가 없어** 클라이언트만으로 만들 수 없었다
(2026-09-02, `09b5040` 기준). 서버를 먼저 세 커밋으로 붙였다.

| 기능 | 엔드포인트 | 상태 |
|---|---|---|
| 프로필 조회·수정 | `GET`/`PATCH /users/me` | ✅ |
| 로그인·가입·구글·토큰 갱신 | `POST /auth/register`·`login`·`google`·`refresh` | ✅ |
| **회원 탈퇴** | `DELETE /users/me` | ✅ **추가됨** (`2961275`) |
| 비밀번호 변경 | — | ❌ 없음 (탈퇴와 무관하게 아직 없다) |

#### 확인을 두 번 받는다

되돌릴 수 없는 동작이라 `showCuteAlert`를 두 번 띄운다. 1단계는 **무엇이
사라지는지 실제 개수로** 말하고, 2단계는 복구 불가만 말한다.

개수를 실제 값으로 채우려면 설정 화면이 직접 불러와야 한다. **홈은
`fetchSessions()`만 부르고 `loadRoutines()`는 부르지 않는다**(`index.tsx`).
운동·통계 탭을 한 번도 안 들르고 설정으로 오면 루틴이 빈 배열이라 "루틴
0개"로 보인다. 되돌릴 수 없는 동작의 확인창에서 사라질 데이터를 실제보다
적게 말하는 셈이라 그냥 둘 수 없었다.

0을 그대로 쓰지 않는 이유가 하나 더 있다. **"아직 안 불러온 0"과 "정말 0"은
다른 상태**인데 문구가 같으면 안 된다. 그리고 둘 다 0일 때 "운동 기록 0개,
루틴 0개가 사라져요"는 잃을 게 없다는 말로 읽힌다 — 기록이 없어도 계정은
사라진다.

| 상태 | 본문 |
|---|---|
| 아직 로딩 중 | 운동 기록과 루틴이 모두 사라져요. |
| 기록 3 · 루틴 2 | 운동 기록 3개, 루틴 2개가 모두 사라져요. |
| 기록만 있음 | 운동 기록 3개가 모두 사라져요. |
| 루틴만 있음 | 루틴 2개가 모두 사라져요. |
| 정말 둘 다 0 | 계정과 저장된 모든 데이터가 사라져요. |

#### 실패하면 화면에 머문다

성공 경로는 로그아웃과 같다 — `logout()` → `replace("/auth/login")`.
**실패하면 로그인 화면으로 보내지 않는다.** 계정이 서버에 남았는데
로그아웃까지 하면 다시 로그인해서 재시도해야 한다. 같은 이유로
`authStore.deleteAccount()`는 서버 삭제만 하고 캐시 정리는 호출부에 맡긴다.

#### 다른 기기에 남은 토큰

한 기기에서 탈퇴해도 다른 기기의 토큰은 살아 있다. 그 기기가 앱을 켜면
`loadToken`의 `GET /users/me`가 **404**를 받는다. 그때 캐시를 버리고 로그인
화면으로 보낸다.

전역 인터셉터에 넣지 않았다. 다른 엔드포인트의 정상적인 404(없는 세션 조회
등)까지 로그아웃이 되기 때문이다. `loadToken`의 catch 한 곳에만 둔다.

**401은 여기서 다루지 않는다.** 전에 있던 `e.response?.status === 401` 검사는
인터셉터가 `ApiError`(`.status`만 있고 `.response`는 없다)로 reject 하기 때문에
**한 번도 참이 된 적이 없는 죽은 조건**이었다. `e.status`로 고쳐 되살리면
오히려 회귀다 — 인터셉터는 운동 세션 중 401을 **일부러 무시**하는데(세션
데이터 유실 방지) 그 401도 여기까지 올라오므로 그 보호가 무력해진다. 나머지
경우는 이미 `_onUnauthorized`가 로그아웃까지 처리한다. 그래서 조건을 되살리지
않고 지웠다.

404에는 같은 세션 보호를 적용하지 않는다. 계정이 없다는 뜻이고, 지킬 서버
데이터가 남아 있지 않다.
...
홈의 "이번 주 운동 0/4"와 "이번 주 자극 부위 0/6"을 조사한 결과다.
**처리 시점: Phase 2 홈 재설계.** A~C를 그때 함께 본다.

분모 4는 `index.tsx`의 `user?.weeklyGoal ?? 4`였다. 서버에 `weeklyGoal`
기본값이 없어 화면 한 줄이 분모를 단독으로 정하고 있었다 — 이 부분은
`user.entity.ts`에 `default: 4`를 주고 마이그레이션으로 운영에 반영해 해결했다.
아래 셋은 남았다.

> 수치는 **2026-09-02(`b8ca26b`) 기준**이다. 이 절의 앞선 판은 `set-target`
> 화면이 있던 시절에 쓰였고, 그 화면이 제거되면서(`b4919c0`) 문제 A가 바뀌었다.

#### 문제 A — 카드를 눌러 도착한 곳에 목표가 없다

설정 경로 자체는 짧아졌다. `edit-profile` 진입점이 **2곳**이다 —
홈 헤더의 프로필 버튼(`index.tsx:481`)과 통계 헤더(`stats.tsx:312`).
홈에서 두 번이면 닿는다.

**그런데 "0/4" 카드를 누르면 거기로 가지 않는다.**

- 카드의 `onPress`는 `router.push("/(tabs)/stats")`(`index.tsx:597`)라
  **통계 탭까지만** 데려다준다
- 그 `stats.tsx`에는 `weeklyGoal`도 "목표"라는 문자열도 **0건**이다
- 즉 목표를 보고 누른 사용자가 도착한 화면에는 그 목표에 대한 언급이 없다

앞선 판이 "4단계"라고 적은 것은 홈 헤더 버튼이 `set-target`으로 가던 때의
경로다. 지금 문제는 단계 수가 아니라 **카드의 목적지가 틀린 것**이다.

> **설정 탭이 생기면서 대안이 하나 늘었다** — 카드의 목적지를 설정 탭(또는
> 그 안의 프로필)으로 바꾸는 것이다. **이번 작업에서는 하지 않았다.**
> 목적지를 무엇으로 할지는 홈 카드가 무엇을 말해야 하는가에 딸린 문제라
> **Phase 2 홈 재설계의 결정**이다. 지금 바꾸면 재설계 때 또 바꾸게 된다.

#### 문제 B — 온보딩이 `weeklyGoal`을 묻지 않는다

`onboarding.tsx`의 저장 페이로드는 `goal / gender / age / height / weight /
isOnboardingDone` 여섯 개뿐이다(`:85-90`). 사용자가 한 번도 정하지 않은 채
기본값이 들어간다.

근본 해결은 온보딩에 한 단계를 더하는 것이다. 그때까지 `weeklyGoal`의 NULL은
"한 번도 정한 적 없음"이라는 신호로 쓸 수 있다 — **그래서 컬럼에 `default: 4`를
주면서도 `nullable`을 유지했고 기존 NULL 레코드를 채우지 않았다.**
(편집 UI의 '미설정'은 표시 전용이다. 값을 비워도 클라이언트가 `undefined`로 보내
키가 누락되므로, 한 번 정한 값은 NULL로 되돌아가지 않는다.)

#### 문제 C — `0/4`와 `0/6`이 형식은 같고 개념이 다르다

| | `0/4` | `0/6` |
|---|---|---|
| 분자 | 이번 주 운동한 **일수** | 이번 주 자극한 **주요 부위 수**(`index.tsx:409`) |
| 분모 | **사용자 목표** `user.weeklyGoal` | **고정 목록의 크기** `MAJOR_MUSCLES.length` |
| 바꿀 수 있나 | 예 — 프로필 편집 | 아니오 |
| 색 | `c.primary` | `c.textSecondary` |

`MAJOR_MUSCLES`(`index.tsx:28`)는 `chest / upper-back / deltoids / abs /
quadriceps / gluteal` 여섯이다. **전체 부위 수가 아니다** —
`MuscleMap.tsx:8`의 `MUSCLE_LABELS`는 **15개**를 정의하고 그중 손으로 여섯을
골랐다. **선정 기준은 주석으로도 남아 있지 않다.**
(앞선 판은 16개로 적었다. 다시 세어 15다.)

화면에서는 둘 다 `N/M` 형식으로 나란히 놓인다. 왼쪽은 "내 목표 대비", 오른쪽은
"고정 목록 대비"인데 형식이 같아 구분이 되지 않는다.
...
### 홈·통계 본문의 타이포 스케일이 DESIGN.md 7역할을 벗어난다

`constants/typography.ts` (2026-09-07, `c6a9b83` 이후). 카드 박스를 없애고
"여백 + 타이포"로 위계를 만들면서 생긴 차이다.

DESIGN.md §3은 타입 역할을 **7개**(22/17/15/14/14/12/11)로 고정하고 *"새 크기를
만들기 전에 굵기(600 → 800 → 900)로 층이 갈리는지 먼저 확인한다"* 고 규정한다.
이 파일은 여섯 개를 새로 들인다.

| 새 크기 | 쓰임 | 굵기로 대체 안 되는 이유 |
|---|---|---|
| 38 / 900 | 주 숫자 | 카드가 사라져 **크기 자체가 위계를 진다.** 22(display)로는 옆 KPI 19와 층이 안 갈린다 |
| 20 / 800 | 주 숫자의 분모 | 38과 15(단위) 사이 |
| 19 / 900 | KPI 값 | 17(title)과 22(display) 사이가 필요 |
| 13.5 / 700 | 리더 행 이름 | 12와 14 사이 |
| 12.5 / 800 | 세그먼트 라벨 | |
| 11.5 / 800 | 키커 라벨 | 11(micro)보다 반 단계 위 |

**DESIGN.md는 직접 편집하지 않는다** — 채택된 계약이고 재컴파일 절차가 따로
있다. 여기 기록만 남기고, 계약에 반영할지는 별도 판단이다.

역설적이지만 이 변경의 목적은 **편차 해소**다. 기존 섹션 제목이 11/14/17/22로
제각각이던 것을 키커 11.5 하나로 모았다. 크기 종류는 늘었지만 **같은 역할에
같은 크기**가 되었다.

### 1px 룰 색은 `c.border` 로 둔다

카드가 사라지면서 구분선이 `c.surface` 위가 아니라 **화면 배경 위**에 놓인다.
대비를 재보면 낮다.

| | 라이트 | 다크 |
|---|---|---|
| `border` on `background` (새 룰) | 1.16:1 | 1.64:1 |
| `border` on `surface` (기존 카드 보더) | 1.26:1 | 1.43:1 |

**기존에 이미 쓰던 것보다 나쁘지 않다.** 다크에서는 오히려 높다(1.64 > 1.43).
헤어라인 구분선은 원래 이 대역이고(iOS 시스템 separator도 1.1~1.3 수준),
WCAG 1.4.11의 3:1은 *"콘텐츠를 이해하는 데 필요한"* 비텍스트 요소에 걸리는
기준이라 장식적 구분선에는 적용되지 않는다.

더 진한 후보(`textMuted` 2.28/3.34)는 구분선이 아니라 보더로 읽힌다 —
카드를 없앤 목적과 반대다. `surfaceHigh`·`surfaceAlt`는 `border`보다 더 흐리다.

### 온보딩에 `weeklyGoal` 단계 추가 — 미해결

현재 신규 사용자는 `weeklyGoal`이 NULL이고 홈이 `?? 4`로 폴백한다. 홈 히어로의
목표 스테퍼로 조정은 가능해졌으나(2026-09-07, `7e43799` 이후), **"한 번도 정한
적 없음" 신호를 온보딩에서 소비하는 것이 본래 자리다.**

`onboarding.tsx`는 2단계(목표 / 성별·나이·키·몸무게)이고 저장 페이로드에
`weeklyGoal`이 없다. 컬럼에 `default: 4`를 주면서도 `nullable`을 유지하고 기존
NULL을 백필하지 않은 이유가 이 신호를 남겨 두기 위해서였다 —
[DEPLOY-BLOCKERS.md] 항목 1의 `AddWeeklyGoalDefault` 마이그레이션 참조.

홈에서 풀 문제가 아니다. 홈은 이미 정해진 값을 조정하는 자리이고, 처음 정하는
자리는 온보딩이다. 폴백 `?? 4`는 그때까지 유지한다.
