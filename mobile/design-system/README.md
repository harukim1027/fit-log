...
### 계정 삭제(탈퇴)가 없다 — 출시 전 필요

설정 탭을 만들면서 계정 구획에 넣을 수 있는 것을 확인한 결과, **서버에
비밀번호 변경도 회원 탈퇴도 엔드포인트가 없다** (2026-09-02, `09b5040` 기준).
...
그래서 설정 탭의 계정 구획은 **이메일 표시와 로그아웃 둘뿐**이다.

**탈퇴는 앱스토어 심사 요구사항이다.** 계정을 만들 수 있는 앱은 앱 안에서
계정 삭제도 할 수 있어야 한다(App Store Review Guidelines 5.1.1(v)).
서버 작업이 선행돼야 하므로 **출시 전 과제**로 남긴다. 클라이언트만으로는
만들 수 없다.
