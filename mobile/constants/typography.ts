 ...
 * @description 홈·통계 본문의 타이포·간격 스케일 ("여백 + 타이포" 레이아웃).
 *
 * ── 왜 상수로 빼는가 ───────────────────────────────────────────────────────
 * 두 화면이 같은 위계를 써야 한다. 각자 인라인 숫자로 쓰면 한쪽만 고쳐졌을 때
 * 어긋나고, 그 어긋남은 코드로 드러나지 않는다. 카드가 사라지면서 위계를
 * 지탱하는 것이 **크기와 여백뿐**이 되었으므로 값이 갈리면 곧바로 무너진다.
 *
...
export const type = {
  ...
  /**
   * 주 숫자의 분모("3/4일"의 "/4").
   * **시안에 없다** — 시안은 통계만 그려 단위만 붙는다. 지시서 값(20/800).
   */
  bigDenominator: { fontSize: 20, fontWeight: "800" },
...
/**
 * 점선 리더 행 (통계 "최고 기록", 홈 "이번 주 기록").
 * 시안 `.B .prow / .pn / .pv / .pd`.
 *
 * 홈의 기록 리스트는 시안에 없어 지시서를 따르되, **볼륨 크기는 시안 `.pv`
 * 의 15/800 에 맞춘다** — 같은 성격의 행이라 둘이 달라야 할 이유가 없다.
 */
export const leaderRow = {
  ...
  dotsMarginH: 4,
  dotsMarginBottom: 4,
  /** 카테고리 색 점. **시안에 없다** — 지시서 값(7). */
  dotSize: 7,
  /** 요일. **시안에 없다** — 지시서 값(11.5 / 600). */
